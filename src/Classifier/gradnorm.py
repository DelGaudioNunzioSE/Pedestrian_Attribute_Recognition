...
def gradNorm(net, layer, alpha, dataloader, num_epochs, lr1, lr2, log=False):
    ...
    validator=Tester(dataloader, 1, 1, 1)
    # start traning
    iters = 0
    net.train()
    for epoch in range(num_epochs):
        ...
        for data in dataloader:
            ...
            # forward pass
            loss, gender, bag, hat = net(*data) #ritorno anche i risultati
            # initialization
            if iters == 0:
                # init weights
            ...
            # update iters
            iters += 1
        validator.test(net,GENDER_LOSS_WEIGHT, BAG_LOSS_WEIGHT,HAT_LOSS_WEIGHT)
    # get logs
    if log:
        return np.stack(log_weights), np.stack(log_loss), gender, bag, hat
