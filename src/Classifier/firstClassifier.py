...
class CNNWithAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.relu(self.conv2(x))  # [B, 128, H/2, W/2]
        x = self.pool(x)              # [B, 128, H/4, W/4]

        x = self.relu(self.conv3(x))  # [B, 256, H/4, W/4]
        ...
        x1 = self.fc1(attn1)  # [B, hidden_dim // 2]
        x2 = self.fc2(attn2)  # [B, hidden_dim // 2]
        x3 = self.fc3(attn3)  # [B, hidden_dim // 2]
