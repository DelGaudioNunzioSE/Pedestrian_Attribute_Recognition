...
def real_to_camera(point_x, point_y, point_z, camera_x, camera_y, camera_z,  theta_x=0, theta_y=0, theta_z=0):
    ...
    rotation_matrix = R_x(theta_x) @ R_y(theta_y) @ R_z(theta_z)  # Assumiamo l'ordine di rotazione Z-Y-X
...
thyaw = 12 * np.pi / 180  # Yaw (rotazione attorno a Z)
thpitch = 10.5 * np.pi / 180  # Pitch (rotazione attorno a Y) (radianti)
throll = -36 * np.pi / 180  # Roll (rotazione attorno a X)
# Parametri immagine
f = 0.00325  # Distanza focale (m)
s_w = 0.00498  # Larghezza sensore (m)
s_h = 0.00374  # Altezza sensore (m), !!!!!!!!!!!!!!!!!!!!!!!!!!
U = 1920   # Larghezza immagine (pixel)
V = 1080  # Altezza immagine (pixel)
...
for i in range(len(x_real)):
    ...
    #invertire y con z 
    XYZ=real_to_camera(point_x=x_real[i], point_y=z_real[i], point_z=y_real[i], camera_x=xt, camera_y=zt, camera_z=yt, theta_x=thpitch, theta_y=0, theta_z=0)
